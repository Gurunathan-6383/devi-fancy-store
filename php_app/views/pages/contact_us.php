...
$contactInfo = [
    ['icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>', 'title' => 'Call Us', 'detail' => '+91 63838 11702', 'sub' => 'Mon-Sat, 9AM - 8PM', 'gradient' => 'from-pink-500 to-rose-500'],
    ['icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>', 'title' => 'Email Us', 'detail' => 'contact@devifancystore.com', 'sub' => 'We reply within 24 hours', 'gradient' => 'from-purple-500 to-indigo-500'],
    ['icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/>', 'title' => 'Visit Us', 'detail' => 'Thiruthangal, Sivakasi', 'sub' => 'Tamil Nadu, India', 'gradient' => 'from-amber-500 to-orange-500'],
    ['icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>', 'title' => 'Business Hours', 'detail' => 'Mon - Sat: 9AM - 8PM', 'sub' => 'Sunday: 10AM - 6PM', 'gradient' => 'from-emerald-500 to-teal-500'],
];
?>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-10 relative z-10">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            ...
            <div class="rounded-2xl p-6 shadow-lg border transition-all hover:-translate-y-1 hover:shadow-xl <?= $dark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100' ?>">
                <div class="w-14 h-14 rounded-xl bg-gradient-to-br <?= $item['gradient'] ?> flex items-center justify-center mb-4 shadow-lg">
                    <svg class="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><?= $item['icon'] ?></svg>
                </div>
                <h3 class="font-heading font-bold text-lg mb-1 <?= $dark ? 'text-white' : 'text-gray-900' ?>"><?= $item['title'] ?></h3>
                <p class="font-medium text-sm <?= $dark ? 'text-gray-200' : 'text-gray-800' ?>"><?= $item['detail'] ?></p>
                <p class="text-xs mt-1 <?= $dark ? 'text-gray-500' : 'text-gray-500' ?>"><?= $item['sub'] ?></p>
            </div>
            <?php endforeach; ?>
        </div>
    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-10">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="rounded-2xl p-6 shadow-lg border <?= $dark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100' ?>">
                    ...
                    <div class="space-y-3">
                        <?php foreach ([['📦', 'Track your order status'], ['🔄', 'Easy returns & exchanges'], ['💬', 'Product recommendations'], ['🤝', 'Bulk order & wholesale inquiries'], ['🎁', 'Gift wrapping available']] as $item): ?>
                        <div class="flex items-center gap-3">
                            <span class="text-xl"><?= $item[0] ?></span>
                            <span class="text-sm <?= $dark ? 'text-gray-300' : 'text-gray-600' ?>"><?= $item[1] ?></span>
                        </div>
                        <?php endforeach; ?>
                    </div>
                </div>
