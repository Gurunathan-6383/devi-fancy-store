...
<div class="min-h-screen bg-gray-50 dark:bg-gray-900">
            ...
            <div class="flex flex-col md:flex-row items-center gap-8">
                <div class="flex-1 text-center md:text-left">
                    <div class="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm rounded-full px-4 py-1.5 mb-4">
                        ...
                        <span class="text-white/80 text-xs font-medium uppercase tracking-wider"><?= count($categories) ?> Categories Available</span>
                    </div>
                    <h1 class="text-4xl md:text-6xl font-heading font-bold text-white mb-4 leading-tight">
                        Find Your <span class="text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 via-pink-400 to-primary-400">Perfect Style</span>
                    </h1>
                    <p class="text-white/60 text-lg max-w-md leading-relaxed">Curated collections for every taste, every occasion, every you.</p>
                </div>
                <div class="hidden lg:flex items-center gap-3">
                    <?php foreach (array_slice($categories, 0, 4) as $i => $cat):
                        $imgUrl = !empty($cat['image']) ? $cat['image'] : ($categoryImages[$cat['slug'] ?? ''] ?? null);
                    ?>
                    <div class="relative w-16 h-16 rounded-2xl overflow-hidden border-2 border-white/20 shadow-lg" style="transform: rotate(<?= ($i - 1.5) * 5 ?>deg)">
                        <?php if ($imgUrl): ?>
                        ...
                        <?php else: ?>
                            <div class="w-full h-full bg-gradient-to-br <?= $gradients[$i % count($gradients)] ?> flex items-center justify-center">
                                <span class="text-2xl"><?= $emojis[$i % count($emojis)] ?></span>
                            </div>
                        <?php endif; ?>
                    </div>
                    <?php endforeach; ?>
    ...
    <!-- Bento Grid -->
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-10 relative z-10 pb-16">
        <?php if (count($categories) > 0): ?>
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 auto-rows-[200px]">
            <?php foreach ($categories as $i => $cat):
                $hasImage = !empty($cat['image']);
                $gradient = $gradients[$i % count($gradients)];
                $emoji = $emojis[$i % count($emojis)];
                $imgUrl = $hasImage ? $cat['image'] : ($categoryImages[$cat['slug'] ?? ''] ?? null);
                $bentoClass = getBentoSize($i);
                $heightClass = getHeight($i);
            ?>
            <a href="<?= $baseUrl ?>/categories/<?= htmlspecialchars($cat['slug'] ?? '') ?>" class="group relative rounded-3xl overflow-hidden <?= $bentoClass ?> <?= $heightClass ?> shadow-lg hover:shadow-2xl transition-all duration-500 hover:-translate-y-1">
                <?php if ($imgUrl): ?>
                    <img src="<?= htmlspecialchars($imgUrl) ?>" alt="<?= htmlspecialchars($cat['name'] ?? '') ?>" class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" loading="lazy" />
                <?php else: ?>
                    <div class="absolute inset-0 bg-gradient-to-br <?= $gradient ?>">
                        <div class="absolute inset-0 flex items-center justify-center">
                            <span class="text-7xl opacity-30 group-hover:opacity-50 group-hover:scale-125 transition-all duration-500"><?= $emoji ?></span>
                        </div>
                    </div>
                <?php endif; ?>
                ...
                <div class="absolute top-0 right-0 w-24 h-24 bg-white/5 backdrop-blur-sm rounded-bl-[3rem] flex items-center justify-center">
                    <span class="text-3xl opacity-60 group-hover:opacity-100 group-hover:rotate-12 transition-all duration-300"><?= $emoji ?></span>
                </div>
                <div class="absolute bottom-0 left-0 right-0 p-5 sm:p-6">
                    <div class="transform group-hover:-translate-y-1 transition-transform duration-300">
                        <h3 class="font-heading font-bold text-white text-xl sm:text-2xl mb-1"><?= htmlspecialchars($cat['name'] ?? '') ?></h3>
                        <div class="flex items-center gap-2 text-white/60 group-hover:text-yellow-300 text-sm font-medium transition-colors">
                            <span>Explore</span>
                            <svg class="w-4 h-4 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg>
                        </div>
                    </div>
                </div>
                <div class="absolute inset-0 rounded-3xl ring-0 group-hover:ring-2 ring-white/30 ring-inset transition-all duration-300"></div>
            </a>
            <?php endforeach; ?>
        </div>
        <?php else: ?>
        <div class="text-center py-20">
            ...
            <h2 class="text-2xl font-heading font-bold mb-2 <?= $dark ? 'text-white' : 'text-gray-900' ?>">No Categories Yet</h2>
            <p class="<?= $dark ? 'text-gray-400' : 'text-gray-500' ?>">Categories will appear here once added by the admin.</p>
        </div>
        <?php endif; ?>
    </div>
</div>
